...
def main(model, T, n_iter, n_batch, n_hidden, capacity, comp, fft):
	...
	# --- Initialization ----------------------
	optimizer = tf.train.RMSPropOptimizer(learning_rate=0.001, decay=0.9).minimize(cost)#0.001
	...
	with tf.Session(config=tf.ConfigProto(log_device_placement=False, allow_soft_placement=False)) as sess:
		...
		while step < n_iter:
			...
			df.to_csv('./' + 'denoise_task_train_GORU.csv')
			step += 1
		...
		df2.to_csv('./' + 'denoise_task_test_GORU.csv')
