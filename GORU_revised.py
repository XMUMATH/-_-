...
class GORUCell(RNNCell):
	...
	def __call__(self, inputs, state, scope=None):
		with vs.variable_scope(scope or "goru_cell"):
			...
			#r = math_ops.maximum(0.2*r_tmp ,r_tmp)
			g = math_ops.sigmoid(g_tmp)
			#g = math_ops.maximum(0.2*g_tmp ,g_tmp)
			...
			new_state = state +  math_ops.multiply(1-g, c)
			mean, variance = tf.nn.moments(new_state, [1])
			beta = tf.get_variable('beta', [1], tf.float32,
                               initializer=tf.constant_initializer(0.0, tf.float32))
			gamma = tf.get_variable('gamma', [1], tf.float32,
                                initializer=tf.constant_initializer(1.0, tf.float32))
			new_state = tf.nn.batch_normalization(new_state, mean, variance, beta, gamma, 0.001, name=None) 
			#new_state_mean = math_ops.reduce_mean(new_state)
			#new_state_var = np.var(new_state)
			#new_state = (new_state-new_state_mean)/(np.sqrt(new_state_var) + 1e-6)
